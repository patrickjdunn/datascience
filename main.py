...
def get_user_inputs():
    time_horizon = input("Enter time horizon (10yr or 30yr): ").strip().lower()
    if time_horizon not in ["10yr", "30yr"]:
        raise ValueError("Invalid time horizon. Choose '10yr' or '30yr'.")

    condition = input("Enter condition (cvd, ascvd, or hf): ").strip().lower()
    if condition not in ["cvd", "ascvd", "hf"]:
        raise ValueError("Invalid condition. Choose 'cvd', 'ascvd', or 'hf'.")

    gender = input("Enter gender (male or female): ").strip().lower()
    if gender not in ["male", "female"]:
        raise ValueError("Invalid gender. Choose 'male' or 'female'.")

    return time_horizon, condition, gender

# Define age
age = 50  # you can also prompt this from user if needed
...
time_horizon, condition, gender = get_user_inputs()
...
def calculate_non_hdl_value(time_horizon, condition, gender, non_hdl_cholesterol):
    ...
    derived_value = calculate_non_hdl_derived(non_hdl_cholesterol)
...
age_non_hdl_coefficients = {
    "10yr_cvd_female": 0.116521,
    "10yr_cvd_male": 0.105339,
    "10yr_ascvd_female": 0.098762,
    "10yr_ascvd_male": 0.111874,
    "10yr_hf_female": 0.102215,
    "10yr_hf_male": 0.113219,
...
#print(f"Age × Non-HDL value: {value:.4f}")
...
age_hdl_coefficients = {
    ...
    "10yr_hf_female": 0,
    "10yr_hf_male": 0,
    "30yr_cvd_female": 0.029076,
    "30yr_cvd_male": 0.0177865,
    "30yr_ascvd_female": 0.0288888,
    "30yr_ascvd_male": 0.0244639,
    "30yr_hf_female": 0,
    "30yr_hf_male": 0
}
...
#print(f"Age × HDL value: {value:.4f}")
...
def evaluate_blood_pressure(hypertension, hypertension_treatment, systolic_blood_pressure, diastolic_blood_pressure):
    # Normalize text inputs
    hypertension = hypertension.strip().capitalize()
    hypertension_treatment = hypertension_treatment.strip().capitalize()

    # Initialize score and message
    bp_score = None
    message = ""

    # 1. Hypertension is "No" and values normal
    if hypertension == "No" and systolic_blood_pressure < 120 and diastolic_blood_pressure < 80 and hypertension_treatment in ["No", "Making lifestyle changes"]:
        bp_score = 100
        message = "Blood pressure goal met"

    # 2. Hypertension is "No"
    elif hypertension == "No":
        if 120 <= systolic_blood_pressure < 130 and diastolic_blood_pressure < 80 and hypertension_treatment in ["No", "Making lifestyle changes"]:
            bp_score = 75
            message = "Your blood pressure is elevated"
        elif 130 <= systolic_blood_pressure < 140 and 80 <= diastolic_blood_pressure < 90 and hypertension_treatment in ["No", "Making lifestyle changes"]:
            bp_score = 50
            message = "You are in hypertension stage 1"
        elif 130 <= systolic_blood_pressure < 140 and 80 <= diastolic_blood_pressure < 90 and hypertension_treatment == "Taking medications":
            bp_score = 30
            message = "Your blood pressure is increasing your health risk"
        elif 140 <= systolic_blood_pressure < 160 and 90 <= diastolic_blood_pressure < 100 and hypertension_treatment in ["No", "Making lifestyle changes"]:
            bp_score = 25
            message = "Your blood pressure is increasing your health risk"
    ...
    # 3. Hypertension is "Yes"
    elif hypertension == "Yes":
        if hypertension_treatment in ["Taking medications", "Making lifestyle changes"]:
            bp_score = 50
            message = "Hypertension goal met"
        elif hypertension_treatment == "No":
            bp_score = 0
            message = "Your blood pressure is increasing your health risk"

    # 4. Specific elevated cases with treatment
    if 120 <= systolic_blood_pressure < 130 and 80 <= diastolic_blood_pressure < 90 and hypertension_treatment == "Taking medications":
        bp_score = 55
        message = "Your blood pressure is elevated"
    elif 140 <= systolic_blood_pressure < 160 and 90 <= diastolic_blood_pressure < 100 and hypertension_treatment == "Taking medications":
        bp_score = 5
        message = "Your blood pressure is increasing your health risk"

    # 5. Hypertension is "No" alone
    if hypertension == "No" and systolic_blood_pressure < 120 and diastolic_blood_pressure < 80:
        bp_score = 100
        message = "Blood pressure goal met"

    return bp_score, message

# Call the function
blood_pressure_score, feedback = evaluate_blood_pressure(hypertension, hypertension_treatment, systolic_blood_pressure, diastolic_blood_pressure)

#display the result
#print(f"Blood Pressure Score: {blood_pressure_score}")
print(f"Blood pressure assessment: {feedback}")

# Function to categorize blood pressure
def categorize_bp(id, datetime, systolic_blood_pressure, diastolic_blood_pressure, symptoms):
    if systolic_blood_pressure >= 180 or diastolic_blood_pressure >= 120:
        if symptoms:
            return "Hypertensive Crisis with Symptoms. This is a medical emergency. Do not wait to see whether your blood pressure comes down on its own. Call 911."
        else:
            return "Hypertensive Crisis without Symptoms. If you do not have any symptoms or any other new and concerning symptoms, you probably have hypertensive urgency. This usually does not need hospitalization, but you should contact your health care professional as soon as possible. Your health care professional may adjust your medications. "
    ...
    elif systolic_blood_pressure < 90 or diastolic_blood_pressure < 60:
        return "Low blood pressure"
    else:
        return "Unclassified blood pressure"

# Call the function
bp_category = categorize_bp(id, datetime, systolic_blood_pressure, diastolic_blood_pressure, symptoms)
...
#print(f"BPTreat Value: {bp_value:.4f}")
...
#print(f"Age × SBP Value: {result:.6f}")
...
#print(f"Diabetes Value: {result:.6f}")
...
#print(f"Age × Diabetes Value: {result:.6f}")
...
def calculate_A1c_glucose_derived_value(time_horizon, condition, gender, A1c, diabetes):
        # Convert diabetes to 1 if "Yes", else 0
        diabetes_derived = 1 if diabetes.lower() == "yes" else 0
        ...
        value = A1c_glucose_derived * coefficient
...
def calculate_A1c_diabetes_derived_value(time_horizon, condition, gender, A1c, diabetes_derived):
        ...
        value = A1c_diabetes_derived * coefficient

        return value

# Derive diabetes_derived from diabetes status
diabetes_derived = calculate_diabetes_derived(diabetes)

# Call the function
A1c_diabetes_value = calculate_A1c_diabetes_derived_value(time_horizon, condition, gender, A1c, diabetes_derived)
...
#print(f"Smoking Value: {result:.6f}")
...
#print(f"Age-Smoking Value: {value:.6f}")
...
#print(f"Min BMI Value: {value:.6f}")
...
#print(f"Max BMI Value: {value:.6f}")
...
#print("age_bmi_value:", round(value, 5))
...
#print("min_egfr_value:", round(value, 5))
...
#print("max_egfr_value:", round(value, 5))
...
#print("age_egfr_value:", round(value, 5))
...
def calculate_uacr_value(time_horizon, condition, gender, uacr):
        if uacr <= 0:
            raise ValueError("UACR must be greater than 0 to compute log.")
...
# Function to calculate min_sdi_value
def calculate_min_sdi_derived_value(time_horizon, condition, gender, sdi):
        ...
        value = min_sdi_derived * coefficient
...
def calculate_max_sdi_derived_value(time_horizon, condition, gender, sdi):
        ...
        value = max_sdi_derived * coefficient
...
intercept_constants = {
    "10yr_cvd_female": -3.860385,
    "10yr_cvd_male": -3.631387,
    "10yr_ascvd_female": -4.291503,
    "10yr_ascvd_male": -3.969788,
    "10yr_hf_female": -4.896524,
    "10yr_hf_male": -4.663513,
    "30yr_cvd_female": -1.748475,
    "30yr_cvd_male": -1.504558,
    "30yr_ascvd_female": -2.314066,
    "30yr_ascvd_male": -1.985368,
    "30yr_hf_female": -2.642208,
    "30yr_hf_male": -2.425439
}

# Function to calculate final risk score sum (including intercept)
def calculate_risk_score_sum(
    time_horizon,
    condition,
    gender,
    *values  # unpacked list of all component values
):
    key = f"{time_horizon.lower()}_{condition.lower()}_{gender.lower()}"
    ...
    total = intercept + sum(values)
    return total

# Final calculation of risk_score_sum
...
# Print the risk score sum
#print(f"Risk Score Sum: {risk_score_sum:.6f}")

# Apply logistic transformation to get final risk probability
risk_score = math.exp(risk_score_sum) / (1 + math.exp(risk_score_sum))
#print(f"PREVENT Risk: {risk_score * 100:.1f}%")
